<img
  src="https://komarev.com/ghpvc/?username=arcadi4&color=lightgrey&style=flat-square&label=+Views+Since+2026/03+"
  alt="Profile View Count"
/>

## @4rcadia

```text
Engineering is an aesthetic.
```

- Student
- Full stuck developer
- Hobbyist designer
- macOS gamer

### Skills <br/> 技能

> Languages and frameworks I know or have used. \
> 我使用的语言和框架。

[![Developer skills](https://skillicons.dev/icons?i=c,cpp,go,bash,git)](https://skillicons.dev)
<br/>
[![Developer skills](https://skillicons.dev/icons?i=js,ts,react,next,tailwind,vercel,cloudflare)](https://skillicons.dev)

> Things I'm learning (by priority). \
> 我正在学习的技能（优先级顺序）。

[![Developer skills](https://skillicons.dev/icons?i=rust,postgresql,zig,haskell,ocaml,solidity)](https://skillicons.dev)

---

> Lately I've been working mostly in Figma or Affinity. \
> 最近我基本上只用 Figma 或者 Affinity。

[![Design apps](https://skillicons.dev/icons?i=ai,ps,xd,figma)](https://skillicons.dev)

---

> Other tools and apps. \
> 其他我在用的工具和软件。

[![Other apps](https://skillicons.dev/icons?i=apple,ubuntu,vscode,webstorm,notion,obsidian,latex,r,unity)](https://skillicons.dev)

### Pinned Project <br/> 近期项目

Check out <span><sub><a href="https://github.com/mclucy/lucy"><img src="https://img.shields.io/badge/GitHub-Lucy-181717?logo=github" alt="Lucy Repo" height="16"/></a></sub></span>, the project I'm currently working on! `lucy` is a package manager for Minecraft servers. With an npm-like experience, you can manage and redistribute your servers or modpacks more easily than ever before.

我最近在写 <span><sub><a href="https://github.com/mclucy/lucy"><img src="https://img.shields.io/badge/GitHub-Lucy-181717?logo=github" alt="Lucy Repo" height="16"/></a></sub></span>，`lucy`是一个强大、现代化的 Minecraft 服务器管理工具。它以包管理器的方式处理服务器。高效、简洁、稳定，而且可以轻松地再分发。

Star ⭐️ if you like it! \
给个Star谢谢喵～

### Personal Page <br/> 个人页

Still working on the site design. \
我还在设计网页。

I <small>(will probably)</small> write about programming, design, video games, philosophy, and more. \
我<small>(大概会)</small>写一些关于技术、设计、游戏、哲学的文章。

→ <https://blog.arcadia.moe>

### Stats <br/> 统计数据

<div align="center">

<div>
  <img
    src="https://github-readme-stats-fast.vercel.app/api?username=arcadi4&theme=gruvbox&show_icons=true&hide_border=true"
    alt="Statistics"
    width="59.5%"
  />
  <img
    src="https://github-readme-stats-fast.vercel.app/api/top-langs/?username=arcadi4&theme=gruvbox&show_icons=true&hide_border=true"
    alt="Languages"
    width="26%"
  />
</div>

---

<details>

<summary align=left><h4>Commit history<br/>贡献历史</h4></summary>

<img
  src="https://streak-stats.demolab.com?user=arcadi4&theme=gruvbox&hide_border=true"
  alt="Commit Streak"
  width="70%"
/>

<img
  src="https://github-readme-activity-graph.vercel.app/graph?username=arcadi4&theme=gruvbox&hide_border=true"
  alt="Commit Activity"
  width="70%"
/>

</details>

---

<details>

<summary align=left><h4>Code time<br/>编码时长</h4></summary>

<img
  src="https://wakatime.com/share/@4rcadia/4a26951f-5d17-48f7-a787-a22fa51482fa.svg"
  alt="WakaTime Language Stats"
  width="60%"
/>
<img
  src="https://wakatime.com/share/@4rcadia/6f58e64e-287e-4ca0-a9de-9eb36b80d7a2.svg"
  alt="WakaTime Editor Stats"
  width="60%"
/>

<!-- <img
  src="https://wakatime.com/share/@4rcadia/9f5b5e8d-4344-4bde-aac4-e694209598a9.svg"
  alt="WakaTime Coding Calendar"
/> -->

*Above WakaTime stats is since Jan 18, 2026 \
以上 WakaTime 数据自 2026/01/18 开始统计*

</details>

</div>
